...
using ViveHandTracking;
    ...
    class ThumbLeft : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                Koreographer.Instance.RegisterForEvents(eventID, JudgeThumb);
                isGetPlayOnce = false;
            }
        ...
        private void JudgeThumb(KoreographyEvent koreographyEvent)
        //private void JudgeTiao(KoreographyEvent koreographyEvent, int sampleTime, int sampleDelta, DeltaSlice deltaSlice)
        {
            if(koreographyEvent.HasIntPayload() && koreographyEvent.GetIntValue() == 3)
            {   
                GetFeature();
                Judge_thumb();
                if(isTrue)
